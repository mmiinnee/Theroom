...
class BookingFragmentViewModel(
    ...
    val bookingRoomLiveData: MutableLiveData<Boolean> = MutableLiveData()
        ...
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                when (val result = insertBookingRoomUserCase.execute(bookingEntity)) {
                    is Result.Success -> {
                        when (val resultStatusRoom = updateStatusRoomUserCase.execute(bookingEntity)) {
                            is Result.Success -> {
                                bookingRoomLiveData.postValue(resultStatusRoom.data)
                            }
                            is Result.Fail -> {
